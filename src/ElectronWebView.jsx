  ...
  componentDidMount() {
    ...
    this.view.addEventListener('did-attach', (...attachArgs) => {
      ...
      events.forEach((event) => {
        ...
        this.view.addEventListener(event, this.props[propName])
      })
  ...
  componentWillUnmount() {
    if (this.ready) {
      events.forEach((event) => {
        ...
        this.view.removeEventListener(event, this.props[propName])
      })
    }
  }
    ...
    events.forEach((event) => {
      ...
      if (this.props[propName] !== prevProps[propName]) {
        if (prevProps[propName]) {
          this.view.removeEventListener(prevProps[propName])
        }
        if (this.props[propName]) {
          this.view.addEventListener(this.props[propName])
        }
      }
    })
  }
